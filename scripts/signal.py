...
def create_shifted_sine_wave(freq, sampling_rate, total_time_in_secs = None, return_time = False, shift = 0):
    '''Creates a sine wave with the given frequency, sampling rate, and length'''
    ...
    # rate to get each time_step)
    time = np.arange(total_time_in_secs * sampling_rate) / sampling_rate
    time_extended = np.arange((total_time_in_secs * sampling_rate) + shift) / sampling_rate

    
    sine_wave = np.sin(2 * np.pi * freq * time)
    sine_wave_extended = np.sin(2 * np.pi * freq * time_extended)
    
    # Shift Wave
    sine_wave = sine_wave_extended[shift:]
...
def plot_spectrogram(sweep, sampling_rate):
    '''Plot waveforms and spectrograms together  ## NOTE ONLY SPECTROGRAM'''
    
    sweep = convert_to_mono(sweep)
    
    fig = plt.figure(figsize=(15, 9))
    spec = fig.add_gridspec(ncols = 2, nrows = 2, width_ratios = [2, 1], height_ratios = [1, 1])
    plot_title = f"{sampling_rate} Hz audio"

    ax_spectrogram1 = plt.subplot(spec[1, 0])

    specgram_return_data = plot_spectrogram_to_axes(ax_spectrogram1, sweep, sampling_rate, plot_title)
...
def resample_audio(resample_factor, sampling_rate, freq_sweep_44100):
    
    new_sampling_rate = int(sampling_rate / resample_factor)
    freq_sweep_new = freq_sweep_44100[::resample_factor]
    
    print(f"Sampling rate: {sampling_rate} Hz with Nyquist limit {int(sampling_rate / 2)} Hz")
    print(f"New sampling rate: {new_sampling_rate} Hz with Nyquist limit {int(new_sampling_rate / resample_factor)} Hz")
    
    return freq_sweep_new, new_sampling_rate


def non_uniform_quantize(input_file, output_file, quantization_bits):
    # Read the audio data from the input file
    audio_data, sampling_rate = librosa.load(input_file, sr=None)
    
    # Apply non-linear quantization (e.g., mu-law companding)
    mu = 255
    audio_data_mu = np.sign(audio_data) * np.log(1 + mu * np.abs(audio_data)) / np.log(1 + mu)
    
    # Quantize the audio data
    quantized_audio_data = np.round(audio_data_mu * (2**(quantization_bits - 1))) / (2**(quantization_bits - 1))
    
    # Apply inverse mu-law companding
    audio_data_inv_mu = np.sign(quantized_audio_data) * (1 / mu) * ((1 + mu)**np.abs(quantized_audio_data) - 1)
    
    # Save the quantized audio data to the output file
    librosa.output.write_wav(output_file, audio_data_inv_mu, sampling_rate)
    
    return audio_data_inv_mu, sampling_rate


def interpolate_audio(data, samplerate, factor=2, kind='linear'):
    
    x = np.arange(len(data))
    f = interp1d(x, data, kind=kind)
    x_new = np.linspace(0, len(data) - 1, num=len(data) * factor)
    data_interpolated = f(x_new)
    new_sampling_rate = samplerate * factor
    return data_interpolated, new_sampling_rate
